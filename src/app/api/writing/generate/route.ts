...
const Body = z.object({ system: z.string().min(1), user: z.string().min(1).max(20000) });

/** First-draft generation for the Writer. Same server-only adapter pattern. */
export async function POST(req: Request) {
  ...
  try {
    const text = await adapter.generate({ system: parsed.data.system, user: parsed.data.user, signal: req.signal });
